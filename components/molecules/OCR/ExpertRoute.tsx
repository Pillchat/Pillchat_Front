...
const ExpertCameraOCR = () => {
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [isRecognizing, setIsRecognizing] = useState(false);
  const setExportInfo = useSetAtom(ExpertInfoAtom); // Jotai 상태 설정 함수
  ...
  const extractInfo = (text: string) => {
    const nameRegex = /성명\s*[:：]?\s*(\S+)/;
    const licenseIdRegex = /제\s*(\d+)\s*호/;
    const issueDateRegex = /(\d{4})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일/;

    const nameMatch = text.match(nameRegex);
    const IdMatch = text.match(licenseIdRegex);
    const ImportTimeMatch = text.match(issueDateRegex);

    return {
      ExpertName: nameMatch ? nameMatch[1] : "",
      ExpertId: IdMatch ? IdMatch[1] : "",
      ImportTime: ImportTimeMatch ? ImportTimeMatch[1] : "",
    };
  };
  ...
  const captureAndRecognize = async () => {
    ...
    // OCR 결과에서 필요한 정보 추출
    const ExportInfo = extractInfo(data.text);

    // 학생증 정보가 포함되었으면, 다음 단계로 이동
    if (ExportInfo.ExpertName && ExportInfo.ExpertId && ExportInfo.ImportTime) {
      alert("약사 면허증이 인식되었습니다!");
...
export default ExpertCameraOCR;
